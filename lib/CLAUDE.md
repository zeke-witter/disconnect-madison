...
- `siteConfig`: `{ name, description, url, links }` — canonical site identity
- `baseMetadata`: Full `Metadata` object used in root `app/layout.tsx`. Includes OG + Twitter cards and robots. The OG/Twitter image is generated by `app/opengraph-image.tsx`, and favicons come from file conventions (`app/icon.svg`, `app/apple-icon.png`, `public/favicon.ico`) — not declared here.
