    ...
    if (mode === 'list') {
        return (
            <div>
                ...
                {draftResult && (
                    <p className={`text-sm mb-4 ${draftResult.success ? 'text-(--accent-muted)' : 'text-(--accent)'}`}>
                        {draftResult.message}
                        {draftResult.success && (
                            ...
                            <div key={event.id} className="flex items-center justify-between py-4 gap-4">
                                ...
                                {event.published && (
                                    <button
                                        ...
                                        className="shrink-0 text-sm text-(--accent-muted) hover:text-(--accent-muted) transition-colors disabled:opacity-50"
                                    >
